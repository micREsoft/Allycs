...
void DumpMemoryGui::getMemoryList()
{
	...
	for (DWORD i = 0; i < numHandles; i++)
	{
		...
		else
		{
			...
			Allycs::debugLog.log(L"getMemoryList :: GetModuleFileNameExW failed 0x%X", GetLastError());
#endif
		}
	}
...
bool DumpMemoryGui::getMappedFilename( Memory* memory )
{
	...
	if (GetMappedFileNameW(ProcessAccessHelp::hProcess, reinterpret_cast<LPVOID>(memory->address), filename, _countof(filename)) > 0)
	{
