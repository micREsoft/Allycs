...
{
    if (Allycs::config[APIS_ALWAYS_FROM_DISK].isTrue())
    {
	...
	for (unsigned int i = 0; i < moduleList.size();i++)
	{
		...
		Allycs::windowLog.log(L"Module parsing: %s",moduleList[i].fullPath);
...
void ApiReader::handleForwardedApi(DWORD_PTR vaStringPointer,char * functionNameParent, DWORD_PTR rvaParent, WORD ordinalParent, ModuleInfo *moduleParent)
{
...
#ifdef DEBUG_COMMENTS
		Allycs::debugLog.log(L"API_SET_PREFIX_NAME %s %S Module Handle %p addy %p",moduleParent->fullPath, dllName, hModTemp, addy);
#endif
		if (addy != 0)
		{
	...
	if (module != 0) // module == 0 -> can be ignored
	{
		...
		{
#ifdef DEBUG_COMMENTS
			Allycs::debugLog.log(L"handleForwardedApi :: Api not found, this is really BAD! %S",fordwardedString);
#endif
		}
		else
		{
...
BYTE * ApiReader::getHeaderFromProcess(ModuleInfo * module)
{
	...
	{
#ifdef DEBUG_COMMENTS
		Allycs::debugLog.log(L"getHeaderFromProcess :: Error reading header");
#endif
		delete[] bufferHeader;
		return 0;
	}
...
BYTE * ApiReader::getExportTableFromProcess(ModuleInfo * module, PIMAGE_NT_HEADERS pNtHeader)
{
...
#ifdef DEBUG_COMMENTS
		Allycs::debugLog.log(L"Something is wrong with the PE Header here Export table size %d", readSize);
#endif
	...
	if (readSize)
	{
		...
		{
#ifdef DEBUG_COMMENTS
			Allycs::debugLog.log(L"getExportTableFromProcess :: Error reading export table from process");
#endif
...
#ifdef DEBUG_COMMENTS
	Allycs::debugLog.log(L"parseExportTable :: module %s NumberOfNames %X", module->fullPath, pExportDir->NumberOfNames);
#endif
...
#ifdef DEBUG_COMMENTS
		Allycs::debugLog.log(L"parseExportTable :: api %S ordinal %d imagebase " PRINTF_DWORD_PTR_FULL L" RVA " PRINTF_DWORD_PTR_FULL L" VA " PRINTF_DWORD_PTR_FULL, functionName, ordinal, module->modBaseAddr, RVA, VA);
#endif
...
void ApiReader::findApiByModule(ModuleInfo * module, char * searchFunctionName, WORD ordinal, DWORD_PTR * vaApi, DWORD_PTR * rvaApi)
{
	if (isModuleLoadedInOwnProcess(module))
	{
		...
		if (hModule)
		{
			...
			{
#ifdef DEBUG_COMMENTS
				Allycs::debugLog.log(L"findApiByModule :: vaApi == NULL, should never happen %S", searchFunctionName);
#endif
			}
		}
		else
		{
#ifdef DEBUG_COMMENTS
			Allycs::debugLog.log(L"findApiByModule :: hModule == NULL, should never happen %s", module->getFilename());
#endif
		}
	}
	else
...
void ApiReader::parseModuleWithOwnProcess( ModuleInfo * module )
{
	...
	{
#ifdef DEBUG_COMMENTS
		Allycs::debugLog.log(L"parseModuleWithOwnProcess :: hModule is NULL");
#endif
	}
}
...
bool ApiReader::isPeAndExportTableValid(PIMAGE_NT_HEADERS pNtHeader)
{
	...
	{
		Allycs::windowLog.log(L"-> IMAGE_NT_SIGNATURE doesn't match.");
		return false;
	}
	...
	{
		Allycs::windowLog.log(L"-> No export table.");
		return false;
	}
	else
	{
...
ApiInfo * ApiReader::getApiByVirtualAddress(DWORD_PTR virtualAddress, bool * isSuspect)
{
	...
	//is never reached
	Allycs::windowLog.log(L"getApiByVirtualAddress :: There is a api resolving bug, VA: " PRINTF_DWORD_PTR_FULL, virtualAddress);
	for (size_t c = 0; c < countDuplicates; c++, it1++)
	{
		apiFound = (ApiInfo *)((*it1).second);
		Allycs::windowLog.log(L"-> Possible API: %S ord: %d ", apiFound->name, apiFound->ordinal);
	}
	return (ApiInfo *) 1; 
}
...
void ApiReader::setMinMaxApiAddress(DWORD_PTR virtualAddress)
{
	...
	{
#ifdef DEBUG_COMMENTS
		Allycs::debugLog.log(L"virtualAddress %p < minApiAddress %p", virtualAddress, minApiAddress);
#endif
...
void  ApiReader::readAndParseIAT(DWORD_PTR addressIAT, DWORD sizeIAT, std::map<DWORD_PTR, ImportModuleThunk> &moduleListNew)
{
	...
	{
#ifdef DEBUG_COMMENTS
		Allycs::debugLog.log(L"ApiReader::readAndParseIAT :: error reading iat " PRINTF_DWORD_PTR_FULL, addressIAT);
#endif
	}
...
void ApiReader::parseIAT(DWORD_PTR addressIAT, BYTE * iatBuffer, SIZE_T size)
{
	...
	{
		//Allycs::windowLog.log(L"%08X %08X %d von %d", addressIAT + (DWORD_PTR)&pIATAddress[i] - (DWORD_PTR)iatBuffer, pIATAddress[i],i,sizeIAT);

        if (!isInvalidMemoryForIat(pIATAddress[i]))
        {
#ifdef DEBUG_COMMENTS
			Allycs::debugLog.log(L"min %p max %p address %p", minApiAddress, maxApiAddress, pIATAddress[i]);
#endif
...
#ifdef DEBUG_COMMENTS
				Allycs::debugLog.log(L"apiFound %p address %p", apiFound, pIATAddress[i]);
#endif
                if (apiFound == 0)
                {
                    Allycs::windowLog.log(L"getApiByVirtualAddress :: No Api found " PRINTF_DWORD_PTR_FULL, pIATAddress[i]);
                }
                if (apiFound == (ApiInfo *)1)
                {
#ifdef DEBUG_COMMENTS
                    Allycs::debugLog.log(L"apiFound == (ApiInfo *)1 -> " PRINTF_DWORD_PTR_FULL, pIATAddress[i]);
#endif
                }
                else if (apiFound)
                {
                    countApiFound++;
#ifdef DEBUG_COMMENTS
                    Allycs::debugLog.log(PRINTF_DWORD_PTR_FULL L" %s %d %s", apiFound->va, apiFound->module->getFilename(), apiFound->ordinal, apiFound->name);
#endif
	...
	Allycs::windowLog.log(L"IAT parsing finished, found %d valid APIs, missed %d APIs", countApiFound, countApiNotFound);
}
...
bool ApiReader::addFunctionToModuleList(ApiInfo * apiFound, DWORD_PTR va, DWORD_PTR rva, WORD ordinal, bool valid, bool suspect)
{
	...
	if ((*moduleThunkList).size() > 1)
	{
		...
		while (iterator1 != (*moduleThunkList).end())
		{
			...
			{
#ifdef DEBUG_COMMENTS
				Allycs::debugLog.log(L"Error iterator1 != (*moduleThunkList).end()");
#endif
				break;
			}
		}
	...
	{
#ifdef DEBUG_COMMENTS
		Allycs::debugLog.log(L"ImportsHandling::addFunction module not found rva " PRINTF_DWORD_PTR_FULL, rva);
#endif
		return false;
	}
...
bool ApiReader::isInvalidMemoryForIat(DWORD_PTR address)
{
    ...
    {
        return !(memInfo.State == MEM_COMMIT && ProcessAccessHelp::isPageAccessable(memInfo.Protect));
    }
