...
void PeParser::alignAllSectionHeaders()
{
	...
	if (Allycs::config[DONT_COMPACT_RAW_DATA].isTrue())
	{
		// Dont compact raw data instead keep VA/VS as RA/RS
		for (WORD i = 0; i < getNumberOfSections(); i++)
		{
			...
			// Keep the same virtual and raw addresses/sizes
			listPeSection[i].sectionHeader.PointerToRawData = listPeSection[i].sectionHeader.VirtualAddress;
			listPeSection[i].sectionHeader.SizeOfRawData = listPeSection[i].sectionHeader.Misc.VirtualSize;
		}
	}
	else
	{
		// Original code: compact the raw data
		for (WORD i = 0; i < getNumberOfSections(); i++)
		{
			listPeSection[i].sectionHeader.VirtualAddress = alignValue(listPeSection[i].sectionHeader.VirtualAddress, sectionAlignment);
			listPeSection[i].sectionHeader.Misc.VirtualSize = alignValue(listPeSection[i].sectionHeader.Misc.VirtualSize, sectionAlignment);
