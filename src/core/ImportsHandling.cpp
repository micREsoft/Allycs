...
bool ImportsHandling::addFunctionToModuleList(const ImportThunk * apiFound)
{
	...
	if (moduleListNew.size() > 1)
	{
		...
		while (it_module != moduleListNew.end())
		{
			...
			{
#ifdef DEBUG_COMMENTS
				Allycs::debugLog.log(L"Error iterator1 != moduleListNew.end()");
#endif
				break;
			}
		}
	...
	{
#ifdef DEBUG_COMMENTS
		Allycs::debugLog.log(L"ImportsHandling::addFunction module not found rva " PRINTF_DWORD_PTR_FULL, apiFound->rva);
#endif
		return false;
	}
