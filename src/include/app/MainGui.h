...
public:
	...
	BEGIN_MSG_MAP_EX(MainGui)
		...
		COMMAND_ID_HANDLER_EX(IDC_BTN_DARKMODE, OnDarkMode)
	...
	BEGIN_DLGRESIZE_MAP(MainGui)
		...
		DLGRESIZE_CONTROL(IDC_BTN_DARKMODE,  DLSZ_MOVE_X | DLSZ_MOVE_Y)
...
protected:
	...
	static const int MenuImportsTraceOffsetAllycs = 2;
...
protected:
	...
	HBRUSH OnCtlColorEdit(CDCHandle dc, CWindow wnd);
	HBRUSH OnCtlColorListBox(CDCHandle dc, CWindow wnd);
	HBRUSH OnCtlColorBtn(CDCHandle dc, CWindow wnd);
	HBRUSH OnCtlColorScrollBar(CDCHandle dc, CWindow wnd);
	...
	void OnDarkMode(UINT uNotifyCode, int nID, CWindow wndCtl);
