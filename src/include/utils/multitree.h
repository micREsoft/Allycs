...
public:
	...
	DWORD m_dwTreeStyle;            // Tree styles
	...
	CMultiSelectTreeViewImpl() : m_dwExStyle(0), m_dwTreeStyle(0), m_bMarquee(false)
	{
	}
	...
	void _Init()
	{
		...
		this->ModifyStyle(TVS_SHOWSELALWAYS, 0);
	}
	...
	void _SelectItem(int iIndex, bool bSelect, int action = TVC_UNKNOWN)
	{
		...
		CWindow parent = this->GetParent();
		// Send notifications
		NMTREEVIEW nmtv = { 0 };
		nmtv.hdr.code = TVN_ITEMSELECTING;
		nmtv.hdr.hwndFrom = this->m_hWnd;
		nmtv.hdr.idFrom = this->GetDlgCtrlID();
		nmtv.action = action;
		nmtv.itemNew.hItem = hItem;
		nmtv.itemNew.lParam = this->GetItemData(hItem);
		...
		// Repaint item
		CRect rcItem;
		if( this->GetItemRect(hItem, &rcItem, FALSE) )
			this->InvalidateRect(&rcItem, TRUE);
		// More notifications
	...
	bool _SelectTreeSub(HTREEITEM hItem, HTREEITEM hGoal, int action)
	{
		while( hItem != NULL )
		{
			...
			{
				if( !_SelectTreeSub(this->GetChildItem(hItem), hGoal, action) )
					return false;
			}
	...
	*/
	void _SelectBox(CRect rc)
	{
		...
		while( hItem != NULL )
		{
			...
			{
				CRect rcItem, rcTemp;
				this->GetItemRect(hItem, &rcItem, TRUE);
   ...
   // Message map and handlers
	...
	void OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags)
	{
		if( ::GetKeyState(VK_SHIFT) < 0 )
		{
			...
			case VK_PRIOR:
				...
				this->GetItemRect(m_hExtSelStart, &rcItem1, TRUE);
				this->GetItemRect(hItem, &rcItem2, TRUE);
	...
	{
		this->DefWindowProc(WM_SETFOCUS, (WPARAM)wndOld.m_hWnd, 0);
		...
		this->SendMessage(WM_UPDATEUISTATE, MAKEWPARAM(UIS_CLEAR, UISF_HIDEFOCUS));
	}
	...
	void OnLButtonDown(UINT nFlags, CPoint point)
	{
		...
		TVHITTESTINFO hti = { 0 };
		hti.pt = point;
		HTREEITEM hItem = this->HitTest(&hti);
		if( (hItem == NULL || (hti.flags & TVHT_ONITEMRIGHT) != 0) )
		{
			if( (m_dwExStyle & MTVS_EX_NOMARQUEE) == 0 && ::DragDetect(this->m_hWnd, point) )
			{
		...
		if( m_aData.GetValueAt(iIndex) && (this->GetStyle() & TVS_DISABLEDRAGDROP) == 0 && ::DragDetect(this->m_hWnd, point) )
		{
			NMTREEVIEW nmtv = { 0 };
	...
	{
		HTREEITEM hItem = (HTREEITEM) this->DefWindowProc(uMsg, wParam, lParam);
		if( hItem == NULL )
	...
	DWORD OnItemPrePaint(int /*idCtrl*/, NMCUSTOMDRAW* lpNMCustomDraw)
	{
		...
		if( iIndex >= 0 )
		{
			...
			else
			{
				...
				{			
					COLORREF clrText = this->GetTextColor();
					if( clrText == CLR_NONE )
						clrText = ::GetSysColor(COLOR_WINDOWTEXT);
					COLORREF clrBack = this->GetBkColor();
					if( clrBack == CLR_NONE )
...
public:
   // CHANGE TO THIS:
   DECLARE_WND_SUPERCLASS(_T("WTL_MultiSelectTree"), CTreeViewCtrl::GetWndClassName())
};
