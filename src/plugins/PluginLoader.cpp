...
std::vector<Plugin> & PluginLoader::getAllycsPluginList()
{
	return allycsPluginList;
}
...
bool PluginLoader::findAllPlugins()
{
	...
	if (!allycsPluginList.empty())
	{
		allycsPluginList.clear();
	}
	...
	if (!searchForPlugin(allycsPluginList, dirSearchString, true))
	{
		return false;
	}
...
bool PluginLoader::searchForPlugin(std::vector<Plugin> & newPluginList, const WCHAR * searchPath, bool isAllycsPlugin)
{
	WIN32_FIND_DATA ffd;
	HANDLE hFind = 0;
	DWORD dwError = 0;
	...
	if (dwError == ERROR_FILE_NOT_FOUND)
	{
		...
		Allycs::debugLog.log(L"findAllPlugins :: No files found");
#endif
		return true;
	}
	...
	{
#ifdef DEBUG_COMMENTS
		Allycs::debugLog.log(L"findAllPlugins :: FindFirstFile failed %d", dwError);
#endif
		return false;
	}
	...
	do
	{
		if ( !(ffd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) )
		{
			...
			{
#ifdef DEBUG_COMMENTS
				Allycs::debugLog.log(L"findAllPlugins :: Plugin invalid file size: %s", ffd.cFileName);
#endif
			}
			else
			{
...
#ifdef DEBUG_COMMENTS
				Allycs::debugLog.log(L"findAllPlugins :: Plugin %s", pluginData.fullpath);
#endif
				if (isValidDllFile(pluginData.fullpath))
				{
					if (isAllycsPlugin)
					{
						if (getAllycsPluginName(&pluginData))
						{
							//add valid plugin
						...
						{
#ifdef DEBUG_COMMENTS
							Allycs::debugLog.log(L"Cannot get allycs plugin name %s", pluginData.fullpath);
#endif
						}
					}
					else
...
bool PluginLoader::getAllycsPluginName(Plugin * pluginData)
{
	bool retValue = false;
	char * pluginName = 0;
	def_AllycsPluginNameW AllycsPluginNameW = 0;
	def_AllycsPluginNameA AllycsPluginNameA = 0;

	HMODULE hModule = LoadLibraryEx(pluginData->fullpath, 0, DONT_RESOLVE_DLL_REFERENCES); //do not call DllMain

	if (hModule)
	{
		AllycsPluginNameW = (def_AllycsPluginNameW)GetProcAddress(hModule, "AllycsPluginNameW");

		if (AllycsPluginNameW)
		{
			wcscpy_s(pluginData->pluginName, AllycsPluginNameW());

#ifdef DEBUG_COMMENTS
			Allycs::debugLog.log(L"getPluginName :: Plugin name %s", pluginData->pluginName);
#endif
		...
		{
			AllycsPluginNameA = (def_AllycsPluginNameA)GetProcAddress(hModule, "AllycsPluginNameA");

			if (AllycsPluginNameA)
			{
				pluginName = AllycsPluginNameA();

				StringConversion::ToUTF16(pluginName, pluginData->pluginName, _countof(pluginData->pluginName));

#ifdef DEBUG_COMMENTS
				Allycs::debugLog.log(L"getPluginName :: Plugin name mbstowcs_s %s", pluginData->pluginName);
#endif
	...
	{
#ifdef DEBUG_COMMENTS
		Allycs::debugLog.log(L"getPluginName :: LoadLibraryEx failed %s", pluginData->fullpath);
#endif
		return false;
	}
}
...
bool PluginLoader::buildSearchString()
{
	...
	{
#ifdef DEBUG_COMMENTS
		Allycs::debugLog.log(L"buildSearchString :: GetModuleFileName failed %d", GetLastError());
#endif
		return false;
	}
...
#ifdef DEBUG_COMMENTS
	Allycs::debugLog.log(L"dirSearchString final %s", dirSearchString);
#endif
...
bool PluginLoader::isValidImprecPlugin(const WCHAR * fullpath)
{
	...
	{
#ifdef DEBUG_COMMENTS
		Allycs::debugLog.log(L"isValidImprecPlugin :: LoadLibraryEx failed %s", pluginData->fullpath);
#endif
		return false;
	}
}
