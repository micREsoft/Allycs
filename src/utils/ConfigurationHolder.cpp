...
ConfigurationHolder::ConfigurationHolder(const WCHAR* fileName)
{
    ...
    config[DARK_MODE]                   =   Configuration(L"DARK_MODE",                  Configuration::Boolean);
    config[DONT_COMPACT_RAW_DATA]       =   Configuration(L"DONT_COMPACT_RAW_DATA",      Configuration::Boolean);
...
bool ConfigurationHolder::buildConfigFilePath(const WCHAR* fileName)
{
	...
	if (!GetModuleFileName(0, configPath, _countof(configPath)))
	{
		...
		Allycs::debugLog.log(L"buildConfigFilePath :: GetModuleFileName failed %d", GetLastError());
#endif
		return false;
	}
